...
inline pros::Distance distRight(13);

// Enum to make names easy to read
enum SensorID { FRONT = 0, BACK = 1, LEFT = 2, RIGHT = 3 };
...
inline double getDistance(int sensor_id) {
  ...
  switch (sensor_id) {
  case FRONT:
    raw_value = distFront.get();
    break;
  case BACK:
    raw_value = distBack.get();
    break;
  case LEFT:
    raw_value = distLeft.get();
    break;
  case RIGHT:
    raw_value = distRight.get();
    break;
  }
